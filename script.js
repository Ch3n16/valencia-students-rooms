...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const targetLabel = document.getElementById('captcha-target-label');

    // Config: Categories and their items
    const categories = {
        'transport': { labelES: 'VEHÍCULOS', labelEN: 'VEHICLES', items: ['🚗', '🚕', '🚙', '🚌', '🚎', '🏎️', '🚓', '🚑', '🚒', '🚐', '🚚', '🚛', '🚜', '🏍️', '🛵', '🚲'] },
        'food': { labelES: 'COMIDA/FRUTA', labelEN: 'FOOD/FRUIT', items: ['🍏', '🍎', '🍐', '🍊', '🍋', '🍌', '🍉', '🍇', '🍓', '🫐', '🍈', '🍒', '🍑', '🥭', '🍍', '🥥'] },
        'animals': { labelES: 'ANIMALES', labelEN: 'ANIMALS', items: ['🐶', '🐱', '🐭', '🐹', '🐰', '🦊', '🐻', '🐼', '🐻‍❄️', '🐨', '🐯', '🦁', '🐮', '🐷', '🐽', '🐸'] }
    };

    let currentTarget = '';

    if (robotCheck) {
        robotCheck.addEventListener('click', (e) => {
            if (!robotCheck.disabled) {
                e.preventDefault();
                showVisualCaptcha();
            }
        });
    ...
    function showVisualCaptcha() {
        const keys = Object.keys(categories);
        const randKey = keys[Math.floor(Math.random() * keys.length)];
        const category = categories[randKey];
        currentTarget = randKey;

        const isEnglish = document.querySelector('.lang-opt[data-lang="en"]').classList.contains('active');
        if (targetLabel) targetLabel.textContent = isEnglish ? category.labelEN : category.labelES;

        const numCorrect = Math.floor(Math.random() * 3) + 3;
        const numDistractors = 9 - numCorrect;

        let gridItems = [];

        for (let i = 0; i < numCorrect; i++) {
            const item = category.items[Math.floor(Math.random() * category.items.length)];
            gridItems.push({ char: item, type: 'correct' });
        }

        const otherKeys = keys.filter(k => k !== randKey);
        for (let i = 0; i < numDistractors; i++) {
            const distKey = otherKeys[Math.floor(Math.random() * otherKeys.length)];
            const distItems = categories[distKey].items;
            const item = distItems[Math.floor(Math.random() * distItems.length)];
            gridItems.push({ char: item, type: 'distractor' });
        }
    ...
    if (btnVerify) {
        btnVerify.addEventListener('click', () => {
            ...
            tiles.forEach(t => {
                ...
                const isCorrect = t.dataset.type === 'correct';
            ...
            if (success && selectedCount > 0) {
                ...
                document.getElementById('lbl-robot').innerHTML = '<span style="color:var(--success)">✓ Verificado / Verified</span>';
            } else {
                captchaMsg.textContent = '✗ Incorrecto / Try again';
                setTimeout(showVisualCaptcha, 800);
            }
        });
    }

    window.addEventListener('click', (e) => {
        if (captchaModal && e.target === captchaModal) {
            captchaModal.style.display = 'none';
        }
    });
